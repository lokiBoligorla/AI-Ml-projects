...
To run this project on modern Python versions (such as Python 3.10 up to 3.13+), perform the following tasks:

### 1. Create a Virtual Environment and Activate It (Recommended)
Open your terminal in the `AI-Resume-Analyzer` root folder and run:
...
### 2. Install Modern Compatibility Requirements
Install the modernized dependencies optimized for newer Python environments:
```bash
pip install -r App/requirements_modern.txt
```

### 3. Download Natural Language Processing Assets
Download the spaCy language model to enable entity recognition:
```bash
python -m spacy download en_core_web_sm
```

### 4. Run the Streamlit Application
Navigate into the `App` directory and launch the server:
```bash
cd App
...
> [!NOTE]
> **Dynamic Database Fallback**: The application will attempt to connect to a local MySQL server first. If a MySQL instance is not running or credentials differ, it will **automatically and seamlessly fall back to a local SQLite database (`cv.db`)** without crashing, allowing you to run the app out-of-the-box!
