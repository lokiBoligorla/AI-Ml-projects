...
# Support both pdfminer.six and legacy pdfminer3 safely
try:
...
# Custom SQLite database wrappers to mimic PyMySQL compatibility for legacy MySQL queries
class SQLiteCursorWrapper:
    def __init__(self, sqlite_cursor):
        self.cursor = sqlite_cursor

    def execute(self, sql, parameters=None):
        # 1. Replace %s with ? (SQLite placeholder format)
        sql = sql.replace('%s', '?')
        
        # 2. Handle convert(column using utf8) -> column (MySQL specific syntax)
        sql = re.sub(r"convert\(([\w_]+)\s+using\s+utf8\)", r"\1", sql, flags=re.IGNORECASE)
        
        # 3. Handle CREATE DATABASE IF NOT EXISTS CV; (Not needed in SQLite)
        if "CREATE DATABASE" in sql.upper():
            return
            
        # 4. Handle MySQL table definitions and auto-increment parameters
        if "CREATE TABLE" in sql.upper():
            if "USER_DATA" in sql.upper():
                sql = """
                ...
                """
            elif "USER_FEEDBACK" in sql.upper():
                sql = """
        ...
        # In SQLite, passing 0 for an autoincrement ID will set the ID to 0, which breaks subsequent inserts.
        # Replacing 0 with NULL will correctly trigger SQLite AUTOINCREMENT.
...
class SQLiteConnectionWrapper:
    def __init__(self, db_path='cv.db'):
...
# Patch pandas read_sql to work nicely with our custom connection wrapper
original_read_sql = pd.read_sql
def custom_read_sql(sql, con, *args, **kwargs):
    if hasattr(con, 'conn'):
        sql = re.sub(r"convert\(([\w_]+)\s+using\s+utf8\)", r"\1", sql, flags=re.IGNORECASE)
        return original_read_sql(sql, con.conn, *args, **kwargs)
...
try:
    ...
    # MySQL connection failed, fall back to SQLite cv.db
    connection = SQLiteConnectionWrapper('cv.db')
    cursor = connection.cursor()
    using_sqlite = True
...
def run():
    # Show dynamic SQLite fallback info banner if active
    if 'using_sqlite' in globals() and using_sqlite:
        st.info("ℹ️ Running with local SQLite fallback (MySQL server connection was not available). Your data will be saved locally in `cv.db`.")
    
    # Beautiful responsive typography instead of the branded logo image
    st.markdown("""
        <div style="text-align: center; margin-top: -30px; margin-bottom: 20px;">
            <h1 style="
                ...
                font-size: 3.5rem;
                font-weight: 800;
                background: linear-gradient(135deg, #021659 0%, #0c359e 50%, #4675e8 100%);
                ...
                filter: drop-shadow(0px 2px 4px rgba(2, 22, 89, 0.1));
            ">
                AI Resume Analyzer
            </h1>
            <p style="
            ...
            ">
                Smart Parsing & Skill Recommendations
            </p>
        </div>
