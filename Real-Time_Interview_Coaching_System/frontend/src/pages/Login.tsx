...
const Login = () => {
  ...
  const testCamera = async () => {
    ...
    try {
      ...
      toast.error("Camera access denied (hardware missing or blocked)");
    }
  ...
  const testMicrophone = async () => {
    ...
    try {
      ...
      toast.error(`Microphone access denied: ${error?.message || error?.name || "Unknown error"}`);
    }
    setTesting(prev => ({ ...prev, microphone: false }));
  };

  const canProceed = permissions.microphone === "granted";
        ...
        {!currentUser ? (
          ...
          <>
            ...
            <Button
              ...
              onClick={() => navigate("/speech-test")}
            >
              Skip permissions check & proceed anyway ➜
            </Button>
            {permissions.camera === "denied" && permissions.microphone === "granted" && (
              <p className="text-xs text-orange-500 text-center mt-2 font-medium">
                ⚠️ No camera detected/access denied. You can still proceed to the Speech Test. Camera-based body language analysis will be skipped during the interview.
              </p>
            )}
          </>
        )}
